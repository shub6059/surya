 ...
 *
 * @since Catch Breadcrumb 1.5
 */
if ( ! class_exists( 'Catch_Breadcrumb_Json_Ld_Schema' ) ) :
	class Catch_Breadcrumb_Json_Ld_Schema {
		private $position        = 0;
		...
		function catch_breadcrumb_custom_breadcrumbs_json_ld( $show_on_home ) {

			/* === OPTIONS === */
			$text['home']     = esc_html__( 'Home', 'catch-breadcrumb' ); // text for the 'Home' link
			$text['category'] = esc_html__( 'Archive for ', 'catch-breadcrumb' ); // text for a category page
			$text['search']   = esc_html__( 'Search results for: ', 'catch-breadcrumb' ); // text for a search results page
			$text['tag']      = esc_html__( 'Posts tagged ', 'catch-breadcrumb' ); // text for a tag page
			$text['author']   = esc_html__( 'View all posts by ', 'catch-breadcrumb' ); // text for an author page
			...
			if ( is_front_page() ) {
				...
				if ( is_home() ) {
					...
					if ( 0 != $this_cat->parent ) {
						$parents = get_ancestors( $this_cat->term_id, 'category' );
					...
					$this->breadcrumb_list[] = $this->add_crumbs( $text['search'] . esc_html( get_search_query() ) );
		...
		} // end catch_breadcrumb_breadcrumb_lists
		...
		function json_ld_breadcrumbs() {
			$options = catch_breadcrumb_get_options(); // Get options

			$show_on_home = ( isset( $options['breadcrumb_display_home'] ) && $options['breadcrumb_display_home'] ) ? 1 : 0;
			$schema       = $this->catch_breadcrumb_custom_breadcrumbs_json_ld( $show_on_home );
			$json         = '';
...
new Catch_Breadcrumb_Json_Ld_Schema();

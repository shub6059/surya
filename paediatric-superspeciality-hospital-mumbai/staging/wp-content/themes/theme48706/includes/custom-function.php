...
// Banners
if (!function_exists('banners_shortcode')) {
	function banners_shortcode($atts, $content = null) {
		...
		switch ($link) {
			...
			foreach ( $posts as $j => $post ) {
				...
				$output .= '<li class="'. $spans .'">';
					...
					$output .= '<h5><a href="'.home_url()."/book-an-appointment/".'" title="'.get_the_title($post_id).'">';
					...
					$output .= '<a href="'.get_permalink($post_id).'" class="btn btn-primary" title="'.get_the_title($post_id).'">Read More</a>'.'</div>';
